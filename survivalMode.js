...
function shoot360() {
  ...
  activePowerUpText = '360° Shot!';
  powerUpTextTimer = 180; // ~3 seconds
}
...
function update() {
  ...
  powerUps.forEach((pu, i) => {
    ...
    if (isColliding(player, pu)) {
      activePowerUpText = `${pu.type.toUpperCase()} Activated!`;
      powerUpTextTimer = 300; // display ~5s
      switch (pu.type) {
  ...
  if (powerUpTextTimer > 0) {
    ...
    ctx.fillText(activePowerUpText, canvas.width/2, 60);
    powerUpTextTimer--;
  }
